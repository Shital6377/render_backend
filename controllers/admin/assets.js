...
const destroy = (async (req, res) => {
    ...
    session.startTransaction();
    try {
        await asset_model_1.default.deleteMany({ _id: req.query.id, });
        const responseData = {
...
const changeStatus = (async (req, res) => {
    ...
    try {
        ...
        const assetsData = await asset_model_1.default.findOne({ _id: id });
        assetsData.is_active = status;
        await assetsData.save();
        const message = `Assets status ${(status === "true") ? 'Approved' : 'Rejected'} successfully`;
        const responseData = {
            message: message,
            data: true,
        };
...
const store = (async (req, res) => {
    ...
    try {
        ...
        let message;
        if (id) {
            assetsData = await asset_model_1.default.findOne({ _id: id });
            message = 'Asset' + process.env.APP_UPDATE_MESSAGE;
        }
        else {
            assetsData = await new asset_model_1.default();
            message = 'Asset' + process.env.APP_STORE_MESSAGE;
        }
        assetsData.name = name;
        if (service_type_id) {
            assetsData.service_type_id = new mongoose_1.default.Types.ObjectId(service_type_id);
        }
        await assetsData.save();
        await session.commitTransaction();
